...
| **Padding（内边距）** | **内容区与边框**之间的空白区域，会继承元素的背景（背景色 / 背景图），不可为负。 |
| **Border（边框）**    | 围绕内边距的线条，会占据空间，样式由 `border` 系列属性控制。 |
| **Margin（外边距）**  | **盒子与其他盒子**之间的空白区域，透明且不会继承背景，可设负值（用于调整布局）。 |
...
| `align-items`      | 交叉轴（垂直主轴）对齐               | `center`（居中）、`flex-start`(按起点对齐)、`flex-end`、`stretch`（拉伸填满） |
...
| 二维布局（行 + 列）   | ✅（如卡片墙、复杂页面） | ❌                     |
| 一维布局（仅行 / 列） | ❌                       | ✅（如导航栏、按钮组） |
| 响应式自动换行        | ✅（auto-fit+minmax）    | ✅（flex-wrap）        |
| 项目跨行列            | ✅（grid-column/row）    | ❌（需嵌套）           |
| 简单居中 / 均分       | ❌（Flex 更简洁）        | ✅                     |

**核心原则**：

- 一维布局（如导航栏、水平 / 垂直排列的元素）→ 用 Flex；
- 二维布局（如多行列卡片、复杂页面结构）→ 用 Grid；
- 电商产品列表、仪表盘等「多行多列」场景 → Grid 是最优解。

###### Grid 编写思考逻辑（流畅写代码）

1. 先定容器规则

   → 是否响应式？（是：auto-fit + minmax；否：repeat(列数, 宽度)）；

   → 行列间距？（gap设统一值，如 10px/20px）；

   → 容器是否定宽 / 定高？（最大宽度max-width，避免撑满屏幕）。

2. 再定项目规则

   → 是否跨行列？（是：grid-column/row；否：自动填充）；

   → 项目对齐方式？（place-items一键居中）；

   → 项目视觉样式？（背景 / 阴影 / 圆角，最后加）。

3. 最后适配

   → 特殊屏幕是否需要调整行列数？（仅需改grid-template-columns，无需重写布局）。

###### 总结

Grid 布局的核心是「先定义网格容器的行列规则，再让子元素填充」，相比 Flex 更适合复杂的二维场景，且 `auto-fit + minmax` 能实现无媒体查询的响应式，大幅简化代码。

初期可以先记住 3 个核心用法：

1. 固定行列：`grid-template-columns: repeat(3, 1fr)`；
2. 响应式行列：`repeat(auto-fit, minmax(250px, 1fr))`；
3. 可视化布局：`grid-template-areas`（复杂页面）。
...
#### 1. Grid 核心属性层级（从宏观到微观）

Grid 布局的属性分为「容器属性」（控制整体布局）和「子项属性」（控制单个子元素），层级关系如下：

| 层级          | 容器属性（控制整体）                                         | 子项属性（控制单个元素）                                     | 层级说明                                                     |
| ------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 顶层（布局）  | `display: grid`（开启网格）                                  | -                                                            | 基础开关，无此属性则所有 Grid 属性失效                       |
| 轨道定义      | `grid-template-columns/rows`（行列尺寸）`grid-template-areas`（区域命名） | -                                                            | 定义网格的「骨架」，行列数量 / 尺寸 / 区域名称               |
| 区域定位      | -                                                            | `grid-area`（区域 / 行列合并）↓`grid-row`（行起始 / 结束）`grid-column`（列起始 / 结束）↓`grid-row-start/end`/`grid-column-start/end` | `grid-area` 是 `grid-row + grid-column` 的简写；而 `grid-row` 又是 `start/end` 的简写 |
| 自适应 / 间距 | `grid-auto-columns/rows`（隐式轨道）`gap`（行列间距）        | `align-self/justify-self`（子项自身对齐）                    | 补充隐式网格尺寸，或调整子项在单元格内的位置                 |

**关键示例（对应你的仪表盘）**：

```css
.dashboard {
  grid-template-columns: 250px 1fr; /* 容器：定义2列 */
  grid-template-areas: "sider header" "sider main"; /* 容器：命名4个区域 */
}
.sider {
  grid-area: sider; /* 子项：等价于 grid-row: 1/3; grid-column: 1/2; */
}
.card {
  grid-column: 1 / -1; /* 子项：列从第1列到最后一列（占满整行） */
}
```

#### 2. Grid 尺寸继承 / 传递规则

Grid 的尺寸规则核心是「容器决定轨道尺寸，子项适配轨道，不直接继承尺寸」，重点规则：

1. **轨道尺寸优先级**：`grid-template-columns/rows`（显式轨道） > `grid-auto-columns/rows`（隐式轨道） > 内容自适应
2. 尺寸传递逻辑
   - 容器的 `height: 100vh` 会让网格占满视口，子项的轨道尺寸（如 `1fr`）基于容器总尺寸计算；
